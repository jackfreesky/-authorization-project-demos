...
//	}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		SysUser user=(SysUser)principals.getPrimaryPrincipal();
		Integer userId=user.getId();
		//4.基于用户id获取用户对应的权限标识permission (sys_menus)
		List<String> permissions=sysMenuDao.findPermissions(userId);
		if(permissions==null||permissions.size()==0)
			throw new AuthorizationException();
		//5.封装用户权限信息
		SimpleAuthorizationInfo info=new SimpleAuthorizationInfo();
		//set集合底层使用map存储数据，将数据存储到了map对象key的位置
		//set集合中的元素不允许重复，也就是说可以借助set集合对元素进行去重
		Set<String> stringPermissions=new HashSet<>();
		for (int i = 0; i < permissions.size(); i++) {
			if(!StringUtils.isEmpty(permissions.get(i))) {
				stringPermissions.add(permissions.get(i));
